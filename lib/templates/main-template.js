...
MainTemplate.prototype.apply = function(mainTemplate) {
  ...
  mainTemplate.plugin('local-vars', function(source, chunk, hash) {
    ...
    includeJsLoader = mainTemplate.includeJsLoader || false;
    includeCssLoader = false;
    ...
    return this.asString([
      ...
      Client.localVars({
        ...
        includeLoader: includeJsLoader || includeCssLoader,
  ...
  mainTemplate.plugin('require-extensions', function(extensions/*, chunk, hash */) {
    ...
    if (!publicPath && (mainTemplate.includeJsLoader || includeJsLoader || includeCssLoader)) {
      buf.push('if (!' + this.requireFn + '.p) {');
      if (!this.outputOptions.fruitLoops) {
