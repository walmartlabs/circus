...
module.exports.publish = function(options) {
  ...
  function processCSS(file, data, callback) {
    ...
    var tree = Csso.parse(data + '', 'stylesheet');
    updateUrls(circus.published, tree);
    data = Csso.translate(tree);
...
*/
function updateUrls(published, tree) {
  switch(tree[0]) {
  case 'stylesheet':
  case 'ruleset':
  case 'block':
  ...
  case 'uri':
    for(var i = 1; i < tree.length; i++) {
      ...
      case 'raw':
        ...
        item[1] = published[value] ? published[value] + tail : item[1];
        return;
      }
    }
  }
