...
exports.prototype.apply = function(compiler) {
  ...
  compiler.plugin('compilation', function(compilation, params) {
    ...
    compilation.plugin('optimize-tree', function(chunks, modules, callback) {
      compiler.inputFileSystem.readFile('package.json', function(err, _package) {
        /*istanbul ignore next */
        if (err) {
          throw new Error('Unable to read project package.json: ' + err);
        }

        package = JSON.parse(_package.toString());
        callback();
      });
    });
    ...
    compilation.plugin('additional-assets', function(callback) {
      ...
      var json = {modules: {}, routes: {}},
          contextPaths = [{path: compiler.context, name: package.name}];
